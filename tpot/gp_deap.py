...
def score_individual_(temp_features, log, labels, scorers, case_ids):
    calculated = {}
    ...
    if "complexity" in scorers:
        try:
            ...
            for label in caseIds_labels.groupby('label'):
                ...
                weight = len(label[1].case_id.to_list())
                if label[0] == -1:
                ...
                weight_list.append(weight)
            calculated["complexity"] = round(np.average(complexity_list,weights=weight_list), 4)

        except Exception as e:
            calculated["complexity"] = float(1)
